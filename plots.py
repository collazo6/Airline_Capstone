...
def barplot_ratings(dfs):
    ...
    for col in southwest_df._get_numeric_data().columns[1:-2]:
        ...
        if j == 4:
            ...
            ax[i][j].set_ylim([0,1])
        sns.barplot(x = ['Southwest Airlines','American Airlines','Delta Air Lines',
...
def single_countplot(df,col,airline,palette='magma'):
    ...
    OUTPUT:
    countplot of reviews with given rating for attribute (col) for specific airline
    '''
    sns.countplot(df[col], palette = palette)
    plt.title('{} : {}'.format(airline,col.replace('_',' ').title()))
    plt.show()
...
def boxplot_ratings(dfs,col):
    ...
    plotly.offline.plot({
    "data": data,
    "layout": Layout(title="Ratings ({})".format(col.replace('_',' ').title()))
    })
...
def rating_dist(df,airline):
    ...
    for col in df._get_numeric_data().columns[1:-2]:
        ax[i][j].set_title('{} : {}'.format(airline,col.replace('_',' ').title()))
...
def violin_ratings(dfs,col,country = 'United States'):
    ...
    i,j = 0,0
    airlines = ['Southwest Airlines','American Airlines','Delta Air Lines','United Airlines',
                'All Nippon Airways','Japan Airlines','Qatar Airways']
    for ind,df in enumerate(dfs):
        ...
        ax[i][j].set_title('{} : {}'.format(airlines[ind],col.replace('_',' ').title()))
...
def rating_by_year(dfs,rating = 'rating'):
    ...
    for df in dfs:
        ...
        plt.plot(dates,values, '{}{}-.'.format(markers[i],colors[i]), markersize = 12, label = airlines[i])
...
def united_incident(united_df):
    ...
    OUTPUT:
    graph plotting mean rating by month in 2017 highlighting effects of incidents by united
    '''
    sns.set()
